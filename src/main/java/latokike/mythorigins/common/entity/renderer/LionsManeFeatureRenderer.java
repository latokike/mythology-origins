...
public class LionsManeFeatureRenderer<T extends LivingEntity, M extends EntityModel<T> & ModelWithHead> extends FeatureRenderer<T, M> {
    ...
    public LionsManeFeatureRenderer(FeatureRendererContext<T, M> context, EntityModelLoader loader) {
        this(context, loader, 1.0F, 1.0F, 1.0F);
    }

    public LionsManeFeatureRenderer(FeatureRendererContext<T, M> context, EntityModelLoader loader, float f, float g, float h) {
        super(context);
        this.xScale = f;
        this.yScale = g;
        this.zScale = h;
